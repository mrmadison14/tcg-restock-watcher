...
| alicecollectibles.com | Next.js RSC flight scrape (full catalog, 1 request) | Pokémon |
...
- **Scoped-and-filtered store** — set `collections` **and** `filter_collections = true`. Fetches
  only the named collections (so a big multi-hobby store isn't full-crawled) but *still* applies
  the franchise + sealed filter, so mixed collections (e.g. a "Pokémon TCG" collection carrying
  Warhammer/Gundam/figures, or a cross-franchise "pre-orders" page) drop everything non-matching.
  The `<franchise>:` prefix is advisory here — the franchise filter re-derives it per product.
